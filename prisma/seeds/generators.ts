...
export function createRandomUser(prisma: PrismaClient, role: UserRole) {
  ...
  const email = faker.internet
    .email({ firstName, lastName, provider: "ps-dev.com.pl" })
    .toLowerCase();
  return {
    email,
...
export function createRandomLawFirm(prisma: PrismaClient) {
  ...
  const descriptionHtml =
    `<p><strong>${realisticFirm.tagline}</strong></p><p>${realisticFirm.opis}</p>` +
    faker.lorem
      .paragraphs(2, "\n\n")
      .split("\n\n")
      .map((p) => `<p>${p}</p>`)
  ...
  return {
    ...
    logo: faker.image.personPortrait({ sex: "male", size: 256 }),
    ...
    filmYouTube: faker.datatype.boolean()
      ? "https://www.youtube.com/watch?v=quC2GkURViU"
      : "",
    ...
    pakietSubskrypcji: faker.helpers.arrayElement(
      Object.values(SubscriptionPackage),
    ),
...
export function createRandomCase(prisma: PrismaClient) {
  return {
    ...
    preferowanyKontakt: faker.helpers.arrayElement(
...
export function createRandomOffer(prisma: PrismaClient) {
  ...
  return {
    ...
    opisOferty:
      faker.helpers.arrayElement(descriptions) + " " + faker.lorem.paragraph(),
