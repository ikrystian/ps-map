    ...
    <Card data-score-target="godziny" className="border border-border/30 bg-card/25 backdrop-blur-md rounded-2xl shadow-lg relative overflow-hidden transition-all duration-300">
      ...
      <CardHeader className="border-b border-border/10 pb-4">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <CardTitle className="text-xl text-foreground font-playfair">Godziny otwarcia biura stacjonarnego</CardTitle>
            <CardDescription className="text-muted-foreground text-sm">Zarządzaj standardowymi godzinami pracy Twojego biura</CardDescription>
          </div>
        </div>
      </CardHeader>
      <CardContent className="space-y-4 pt-6">
        <div className="flex items-center space-x-3 p-3 rounded-xl border border-border/20 bg-background/10">
          <Switch
          ...
          />
          <Label htmlFor="statusGodzinyOtwarcia" className="text-foreground cursor-pointer font-medium">
        ...
        {formData.statusGodzinyOtwarcia && formData.godzinyOtwarcia && (
          <div className="grid gap-3 pt-2 animate-in slide-in-from-top-1 duration-200">
            {Object.keys(formData.godzinyOtwarcia).map((day) => {
              ...
              return (
                <div key={day} className="grid grid-cols-1 sm:grid-cols-[160px_1fr_auto_1fr] gap-3 items-center p-2.5 rounded-xl border border-border/10 bg-background/5">
                  <span className="font-semibold text-foreground/80 text-sm sm:pl-2">{dayNamesPolish[day] || day}</span>
                  ...
                  >
                    ...
                    <SelectContent className="bg-background border-border/30 rounded-xl">
                      <SelectItem value="zamkniete" className="text-foreground/80 focus:text-foreground">Zamknięte</SelectItem>
                      {Array.from({ length: 48 }, (_, i) => {
                        const hour = Math.floor(i / 2)
                        const minute = i % 2 === 0 ? "00" : "30"
                        const time = `${hour.toString().padStart(2, "0")}:${minute}`
                        return (
                          <SelectItem key={time} value={time} className="text-foreground/80 focus:text-foreground">
                            {time}
                          </SelectItem>
                        )
                      })}
