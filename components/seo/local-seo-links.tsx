  ...
  const { groups, flatLinks, headingTitle, headingSubtitle } = useMemo(() => {
    ...
    if (fixedCategory && !fixedLocation) {
      ...
      return {
        ...
        headingSubtitle:
          subtitle ??
          `Znajdź eksperta od „${fixedCategory.nazwa}” w swoim regionie`,
      };
    }
    ...
    for (let i = 0; i < shuffledCategories.length && i < maxGroups; i++) {
      ...
      const links = rotated
        .slice(0, linksPerGroup)
        .map((loc) => makeLink(cat, loc));
  ...
  return (
    ...
    >
      <div className="container mx-auto px-4">
        ...
        {groups.length > 0 ? (
          <div className="max-w-6xl lg:nax-w-8xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-8 gap-y-10">
            {groups.map((group) => (
              <div key={group.key}>
                <Link
                  href={group.href}
