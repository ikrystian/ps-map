    ...
    for (const admin of admins) {
      try {
        const { notification } = await sendSystemNotification({
          ...
          tresc: `Użytkownik ${userEmail} zgłosił opinię o tytule "${review.tytulOpinii}" dla kancelarii "${review.lawFirm.nazwa}". Powód: ${readableReason}.${description ? ` Opis: ${description}` : ""}`,
