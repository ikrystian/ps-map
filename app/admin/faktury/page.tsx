        ...
        {invoices.length === 0 ? (
          ...
          <>
            <motion.div variants={itemVariants}>
              <Card variant="glass" className="rounded-2xl shadow-lg relative overflow-hidden">
                ...
                <CardHeader className="border-b border-border/20 py-4 px-6">
                  <CardTitle className="text-lg font-playfair text-foreground">Wszystkie faktury</CardTitle>
                ...
                <CardContent className="p-0">
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow className="border-b border-border/20 hover:bg-transparent">
                          ...
                          <TableHead className="text-muted-foreground font-semibold bg-background/20 text-xs py-3.5 px-6 uppercase tracking-wider">Ekspert</TableHead>
                          ...
                          <TableHead className="text-muted-foreground font-semibold bg-background/20 text-xs py-3.5 px-6 uppercase tracking-wider">Przedmiot</TableHead>
                          <TableHead className="text-muted-foreground font-semibold bg-background/20 text-xs py-3.5 px-6 uppercase tracking-wider">Kwota netto</TableHead>
                          <TableHead className="text-muted-foreground font-semibold bg-background/20 text-xs py-3.5 px-6 uppercase tracking-wider">VAT</TableHead>
                          ...
                          <TableHead className="text-muted-foreground font-semibold bg-background/20 text-xs py-3.5 px-6 uppercase tracking-wider w-44">Status KSeF</TableHead>
                            ...
                            <TableRow key={invoice.id} className="border-b border-border/10 hover:bg-white/[0.02] text-sm text-foreground/80 transition-colors">
                              ...
                              <TableCell className="py-4 px-6">
                                {invoice.order.orderType === "SUBSCRIPTION" ? (
                                  ...
                                  <div>
                                    <div className="font-semibold text-foreground">Pakiet punktów</div>
                                    <div className="text-sm text-muted-foreground font-light">
                                      Zamówienie: {invoice.order.orderNumber}
                                    </div>
                                  </div>
                              ...
                              <TableCell className="py-4 px-6 font-light text-xs text-muted-foreground">
                              ...
                              <TableCell className="py-4 px-6">
                                {invoice.ksefStatus === "ACCEPTED" && (
                                  <div className="space-y-1">
                                    ...
                                    {invoice.ksefNumber && (
                                      <div className="text-sm font-mono text-muted-foreground break-all max-w-[140px]" title={invoice.ksefNumber}>
                                        {invoice.ksefNumber}
                                      </div>
                                    )}
                                  </div>
                              ...
                              <TableCell className="py-4 px-6 text-right">
                                <div className="flex items-center justify-end">
                                  <Button
                                    ...
                                    className="h-9 rounded-lg border border-border/50 text-muted-foreground hover:text-primary hover:bg-primary/5 hover:border-primary/30 transition-all text-xs gap-1.5"
                                  >
                  ...
                  <div className="block md:hidden p-4 space-y-3">
                    {invoices.map((invoice) => {
                      ...
                      return (
                        <div key={invoice.id} className="p-4 rounded-xl border border-border/10 bg-card/40 text-xs space-y-3 relative hover:border-primary/30 transition-all">
                          <div className="flex justify-between items-start gap-2">
                            <div>
                              <h4 className="font-semibold text-foreground text-sm">
                                {invoice.invoiceNumber}
                              </h4>
                              <p className="text-sm text-muted-foreground font-light mt-0.5 flex items-center gap-1">
                                <Building2 className="h-3 w-3 text-muted-foreground" />
                                {invoice.lawFirm?.nazwa || invoice.buyerName}
                              </p>
                              <p className="text-sm text-muted-foreground font-light mt-0.5">{formatDate(invoice.issueDate)}</p>
                            </div>
                          ...
                          <div className="border-t border-border/5 pt-2">
                            <span className="text-sm text-muted-foreground block font-light">Przedmiot</span>
                            ...
                            <p className="text-sm text-muted-foreground">Zamówienie: {invoice.order.orderNumber}</p>
                          </div>

                          <div className="flex justify-between items-center border-t border-border/5 pt-2 text-sm">
                            <div>
                              <span className="text-muted-foreground block font-light">Kwota netto</span>
                              <span className="text-foreground/80 font-medium">{formatCurrency(invoice.netAmount)}</span>
                            </div>
                            <div className="text-right">
                              <span className="text-muted-foreground block font-light">Kwota brutto</span>
                          ...
                          <div className="flex items-center justify-between border-t border-border/5 pt-2.5">
                            <div>
                              <span className="text-muted-foreground block font-light text-sm">Status KSeF</span>
                              <div className="mt-1">
                                {invoice.ksefStatus === "ACCEPTED" && (
                            ...
                            <Button
                              ...
                              size="sm"
                              onClick={() => handleDownload(invoice)}
                              className="h-8 rounded-lg border border-border/50 text-muted-foreground hover:text-primary hover:bg-primary/5 hover:border-primary/30 gap-1 text-sm"
                            >
                              <Download className="h-3.5 w-3.5" />
                              Pobierz PDF
                            </Button>
            ...
            <motion.div variants={itemVariants}>
              <Card variant="glass" className="rounded-2xl shadow-lg relative overflow-hidden">
                <CardHeader className="border-b border-border/20 py-4 px-6">
                  <CardTitle className="text-lg font-playfair text-foreground">Podsumowanie finansowe</CardTitle>
                </CardHeader>
                <CardContent className="p-6">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="p-4 rounded-xl border border-border/10 bg-background/15 flex items-center gap-4">
                      <div className="h-10 w-10 rounded-lg bg-indigo-500/10 border border-indigo-500/20 flex items-center justify-center text-indigo-400">
                        <FileText className="h-5 w-5" />
                      </div>
                      <div>
                        <div className="text-sm text-muted-foreground uppercase tracking-wider font-semibold">Liczba dokumentów</div>
                        <div className="text-2xl font-bold text-foreground tracking-tight">{invoices.length}</div>
                      </div>
                    </div>
                    ...
                    <div className="p-4 rounded-xl border border-border/10 bg-background/15 flex items-center gap-4">
                      ...
                      <div>
                        <div className="text-sm text-muted-foreground uppercase tracking-wider font-semibold">Opłacone faktury</div>
                    ...
                    <div className="p-4 rounded-xl border border-border/10 bg-background/15 flex items-center gap-4">
                      ...
                      <div>
                        <div className="text-sm text-muted-foreground uppercase tracking-wider font-semibold">Łączna kwota brutto</div>
