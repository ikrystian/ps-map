...
export function generateConsultationReminderEmail(
  recipientName: string,
  consultation: any,
  isLawFirm: boolean
): { subject: string; html: string; text: string } {
  const subject = `Przypomnienie o konsultacji prawnej - ${consultation.lawFirm.nazwa}`
  const meetLink = consultation.googleMeetLink
  const consultationDate = new Date(consultation.proposedDateTime).toLocaleString("pl-PL")

  const html = `
    <p>Witaj ${recipientName},</p>
    <p>To jest przypomnienie o nadchodzącej konsultacji prawnej.</p>
    <p><strong>Szczegóły konsultacji:</strong></p>
    <ul>
      <li><strong>Kancelaria:</strong> ${consultation.lawFirm.nazwa}</li>
      <li><strong>Klient:</strong> ${consultation.client.imie} ${consultation.client.nazwisko}</li>
      <li><strong>Data:</strong> ${consultationDate}</li>
    </ul>
    <p><strong>Link do spotkania:</strong> <a href="${meetLink}">${meetLink}</a></p>
    <p>Do zobaczenia!</p>
  `
  const text = `
    Witaj ${recipientName},
    To jest przypomnienie o nadchodzącej konsultacji prawnej.

    Szczegóły konsultacji:
    - Kancelaria: ${consultation.lawFirm.nazwa}
    - Klient: ${consultation.client.imie} ${consultation.client.nazwisko}
